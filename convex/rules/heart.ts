...
export function evaluateHeart(
  ...
  const sodiumLimit = heartDetails?.sodiumLimit
    ? heartDetails.sodiumLimit / 5
    : HEART_THRESHOLDS.highSodium;
  const moderateSodiumLimit = heartDetails?.sodiumLimit
    ? heartDetails.sodiumLimit / 15
    : HEART_THRESHOLDS.moderateSodium;
  ...
  if (sodium >= sodiumLimit) {
    ...
    negativeScore += 3;
    if (!heartDetails?.sodiumLimit && heartDetails?.hasHypertension)
  ...
  const satFatLimit = heartDetails?.saturatedFatLimit
    ? heartDetails.saturatedFatLimit / 5
    : HEART_THRESHOLDS.highSaturatedFat;
